import {
  ...
  export function ExtensionPropertiesProps({ element, injector }) {
    ...
    const items = properties.map((property, index) => {
      ...
      if (property.get('name') || property.get('value')) {
        label = (property.get('name') || '') + ': ' + (property.get('value') || '');
      }
      return {
        id,
        label: label,
  ...
  function addFactory({ bpmnFactory, commandStack, element }) {
    return function(event) {
      ...
      if (!properties) {
        ...
        properties = createElement('simBPMN:Properties', {
          values: []
        }, parent, bpmnFactory);
      ...
      const property = createElement('simBPMN:Property', {}, properties, bpmnFactory);
  ...
  function getProperties(businessObject) {
    return getExtensionElementsList(businessObject, 'simBPMN:Properties')[0];
  }
