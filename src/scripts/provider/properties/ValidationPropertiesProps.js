...
export function ValidationPropertiesProps({ element, injector }) {
  ...
  const items = properties.map((property, index) => {
    const id = element.id + '-validationProperty-' + index;
...
function addFactory({ bpmnFactory, commandStack, element }) {
  return function (event) {
    ...
    if (!properties) {
      ...
      properties = createElement('simBPMN:Validations', {
        values: []
      }, parent, bpmnFactory);
    ...
    const property = createElement('simBPMN:Validation', {}, properties, bpmnFactory);
...
function getProperties(businessObject) {
  return getExtensionElementsList(businessObject, 'simBPMN:Validations')[0];
}
