...
export function AttachmentPropertiesProps({ element, injector }) {
    ...
    const items = properties.map((property, index) => {
        const id = element.id + '-attachmentProperty-' + index;

        let label = '';
        if (property.get('attachmentPath')) {
            let filePath = property.get('attachmentPath');
            label = filePath.replace(/^.*[\\/]/, '')
        }
        return {
            id,
            label: label,
            entries: AttachmentProperty({
                idPrefix: id,
                element,
                property
            }),
...
function addFactory({ bpmnFactory, commandStack, element }) {
    return async function (event) {
        ...
        if (!properties) {
            ...
            properties = createElement('simBPMN:Attachments', {
                values: []
            }, parent, bpmnFactory);
        ...
        let paths = await window.electronAPI.addAttachment();
        let pathProperties = paths.map(path => createElement('simBPMN:Attachment', { attachmentPath: path }, properties, bpmnFactory));
        ...
        commands.push({
            ...
            context: {
                ...
                moddleElement: properties,
                properties: {
                    values: properties.get('values').concat(pathProperties)
                }
            }
        });
...
function getProperties(businessObject) {
    return getExtensionElementsList(businessObject, 'simBPMN:Attachments')[0];
}
