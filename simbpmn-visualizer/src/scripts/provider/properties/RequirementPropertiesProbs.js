import {
  ...
  import RequirementProperty from './RequirementProperty';
  ...
  export function RequirementPropertiesProps({ element, injector }) {
    ...
    const items = properties.map((property, index) => {
      const id = element.id + '-requirementProperty-' + index;

      let label = '';
      if (property.get('name') || property.get('value')) {
        label = (property.get('name') || '') + ': ' + (property.get('description') || '');
      }
      return {
        id,
        label: label,
        entries: RequirementProperty({
          idPrefix: id,
          element,
          property
        }),
  ...
  function addFactory({ bpmnFactory, commandStack, element }) {
    return function(event) {
      ...
      if (!properties) {
        ...
        properties = createElement('simBPMN:Requirements', {
          values: []
        }, parent, bpmnFactory);
      ...
      const property = createElement('simBPMN:Requirement', {}, properties, bpmnFactory);
  ...
  function getProperties(businessObject) {
    return getExtensionElementsList(businessObject, 'simBPMN:Requirements')[0];
  }
